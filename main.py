...
# interpolation/decimation parameters
...
for f in freqs:
    ...
    ups_signal = ups_signal[:len(timeline)]

    # 3.1.1 Upsampling signal in time domain
    plt.subplot(2, 1, 1)
    plt.plot(timeline, ups_signal)
    plt.xlabel("time, s")
    plt.ylabel("A, V")
    ...
    plt.subplot(2, 1, 2)
    plt.stem(np.fft.fftshift(np.fft.fftfreq(len(ups_signal), 1/Fs)), np.abs(np.fft.fftshift(np.fft.fft(ups_signal))))
    plt.xlabel("frequency, Hz")
    plt.ylabel("|S(f)|")
    ...
    plt.subplot(2, 1, 1)
    plt.plot(timeline, interpolate_singal)
    plt.xlabel("time, s")
    plt.ylabel("A, V")
    plt.title(f"Signal AFTER filtering")

    # 3.3.2 Interpolate signal in frequency domain
    interpolation_spectrum = np.abs(np.fft.fftshift(np.fft.fft(interpolate_singal)))
    plt.subplot(2, 1, 2)
    plt.stem(np.fft.fftshift(np.fft.fftfreq(len(interpolate_singal), Ts)), interpolation_spectrum)
    plt.xlabel("frequency, Hz")
    plt.ylabel("|S(f)|")
    ...
    mse = 0
    for i in range(len(interpolate_singal)):
        mse += (signal[i] - interpolate_singal[i])**2
    mse /= len(interpolate_singal)
